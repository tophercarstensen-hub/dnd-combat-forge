...
def _find_etools_root():
    ...
    for p in candidates:
        if (p / "data" / "bestiary").exists():
            return p
    # Last resort: walk Downloads looking for a folder that has data/bestiary
    downloads = Path(r"C:\Users\tophe\Downloads")
    for child in downloads.rglob("bestiary"):
...
OLD_ENRICHED_CANDIDATES = [
    ...
    Path(__file__).parent.parent / "monsters_all_enriched.json",
    ETOOLS_ROOT.parent / "monsters_all_enriched.json",
]

OUTPUT_FILE = Path(__file__).parent / "monsters_all_enriched_v2.json"
# ─────────────────────────────────────────────
