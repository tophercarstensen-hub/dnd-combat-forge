...
V2_FILE      = SCRIPT_DIR / "monsters_all_enriched_v2.json"
OLD_FILE     = SCRIPT_DIR / "monsters_all_enriched.json"
OUTPUT_FILE  = SCRIPT_DIR / "monsters_final.json"
...
def parse_attack_from_actions(actions):
    ...
    for action in (actions or []):
        ...
        dmg_m = dmg_re.search(desc)
        dc_m  = dc_re.search(desc)

        if hit_m and dmg_m:
            try:
                dmg_val = _avg_dice(dmg_m.group(1))
            except Exception:
                dmg_val = 0
            if dmg_val > best_dmg:
                best_dmg = dmg_val
                result["atkBonus"] = int(hit_m.group(1))
                result["dmgPerAtk"] = dmg_m.group(1).replace(" ", "")
        elif dc_m and dmg_m and result["saveDC"] is None:
